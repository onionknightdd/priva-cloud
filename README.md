...
- **Session runtime pool:** keeps up to 5 session-sticky Claude CLI processes resident behind one lifetime receiver each. Capacity is an internal policy computed from the runner container's cgroup memory limit (the current 1639 MiB tier resolves to 5 warm sessions), not a user setting; a separate active-turn budget resolves to 3 on that tier, and live soft/hard watermarks reject automatic peer work, evict idle LRU entries, and pause admission under pressure. Idle entries remain reachable through Claude's native inbox but hold no activity lease, so they do not prevent scale-to-zero; startup-option fingerprints, exclusive delete/rewind reservations, and explicit SIGTERM close-all bound their lifecycle.
- **Permission coordinator:** a generation-scoped, fail-closed bridge binds each pooled runtime's SDK `can_use_tool` callback to only the current run — emits `permission_request`, waits on an `asyncio.Future`, resolves on `POST …/agent/permission/respond`, and cancels outstanding approvals before reuse or eviction.
...
- **MCP:** reads `.mcp.json` (project) + `~/.claude/settings.json` (global); injects the built-in in-process `FileCanvas` server (`register_file`) for the Canvas panel and, for scheduler-enabled runs, the `Scheduler` server with its seven unchanged `scheduler_*` tools.
...
- **Native CLI:** `claude-agent-sdk 0.2.134` bundles Claude Code CLI `2.1.226`; the image exposes that wheel-bundled binary on `PATH`, and the SDK spawns the same binary. Optional cross-session interaction exposes `ListAgents` only to warm pooled sessions and uses `crossSessionInbound=accept`; disabling it denies `ListAgents`/`ListPeers` and refuses inbound peers. The pod runs non-root as uid `10001`.
...
- **Design system:** GitHub Dark palette via CSS variables only (no hardcoded hex, no color Tailwind classes), system UI font stack for product surfaces, JetBrains Mono for code, and Nerd Font for terminal surfaces; no shadows, ≤4px radius, status shown as a 2px left border, skeleton-shimmer loading. Tailwind is layout-only. Cold-start UX: `fetchWithWake()` detects a `503` early (`WAKE_SLOW_MS=900ms`), toasts "Agent sandbox is waking…", and retries up to 6 times with backoff `[1, 2, 4, 8, 16]`s. WebSocket auth carries the JWT in the `Sec-WebSocket-Protocol` subprotocol. See `web/design-spec.md`.
