"""Append-only JSONL log of hook executions for a single-account pod.

Daily-partitioned files live in ``priva_home()``:
``.priva.hooks.log.YYYY-MM-DD.jsonl``.
...
def _daily_path(date_str: str) -> Path:
    return _get_log_dir() / f".priva.hooks.log.{date_str}.jsonl"


def _counts_path() -> Path:
    return _get_log_dir() / ".priva.hooks.log.counts.json"
...
class HookLogStore:
    """Single-pod daily-partitioned JSONL log of hook executions."""
    ...
    def append(self, entry: HookLogEntry) -> None:
        date_str = _entry_date_str(entry)
        path = _daily_path(date_str)
        path.parent.mkdir(parents=True, exist_ok=True)
        counts_path = _counts_path()
    ...
    def _list_daily(self) -> list[tuple[str, Path]]:
        return list_daily_files(_get_log_dir(), _DATE_PATTERN)
    ...
    def _rebuild_counts(self, daily_files: list[tuple[str, Path]]) -> int:
        ...
        write_counts_sidecar(_counts_path(), counts)
        return int(counts["total"])
    ...
    def query_cursor(
        ...
        total = None if event_type else self._get_total()
