...
调用脚本前需要两样东西，都存放在 **`<当前工作目录>/.priva-agent-gateway/`** 下。
...
2. `./.priva-agent-gateway/session.json` 里的 `gateway_url` 字段
...
`./.priva-agent-gateway/auth` 文件，内容为云端 agent sandbox 的 **Bearer token**
...
3. 脚本解析网关地址、读取 `./.priva-agent-gateway/auth`、构建 priva 风格 payload、
   以 SSE 形式调用网关、流式读取响应。
...
p = pathlib.Path("./.priva-agent-gateway/session.json")
