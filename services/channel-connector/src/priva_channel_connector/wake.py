"""Waker — the connector's only Kubernetes surface (copied verbatim from the
scheduler's ``wake.py``). Patch ``AgentTenant spec.wake.requestedAt`` (the ONLY
scale-up trigger — the operator is the sole scaler) and poll the CR status until
the operator reports the pod Running with an IP. RBAC needs exactly
``agenttenants`` get/patch + ``agenttenants/status`` read — no pod verbs, ever."""
