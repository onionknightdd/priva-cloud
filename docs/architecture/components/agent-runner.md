...
The API reference UI is **Scalar** (not Swagger), served **fully offline** from a vendored 3.7MB bundle (`_static/scalar.standalone.js`) with web-fonts disabled (`withDefaultFonts:false`) so the page renders with zero external requests. `docs_url=None` disables FastAPI's Swagger; the pod serves `GET /sandbox/apidocs` (Scalar HTML), `/sandbox/apidocs/scalar.js` (the bundle), and `/sandbox/apidocs/openapi.json` (`openapi_url`). These are **not** routed through the InferencePool — the GIE/EPP response path buffers bodies to ~8KB, which truncates the ~91KB schema (and the 3.7MB bundle). Instead **control-panel serves `/sandbox/apidocs*` by proxying from any Ready runner** (`provisioner.any_ready_runner_endpoint()`; account-independent, full body, cache headers forwarded) on the `/` catch-all, unauthenticated (only the API *shape* is exposed, never user data). The SPA's "API Doc" link (`SettingsPopover`) opens `/sandbox/apidocs` in a new tab. The **same ~8KB EPP truncation** also corrupts large *per-account* reads (session transcripts 35-300KB, file previews ≤1MB, big skill/log/listing bodies — `JSON.parse` chokes on the cut-off body), so control-panel now exposes a **generic** sibling lane on the same `/` catch-all (no `ext_proc`): `GET /api/cp-proxy/<path>` re-does the EPP's per-account steering — auth the bearer token → `wake_and_wait` the caller's pod → mint a **per-account** runner token → GET-only proxy to **this pod's** `/api/sandbox/<path>`, returning the full body (`..` rejected). The SPA points its >8KB-risk reads there (`web/shared/api/client.js` `sandboxRead`, with a 404/network fallback to the direct `/api/sandbox` lane); the original `GET /api/session-history/{id}/messages` proxy is now a thin **back-compat alias** that calls the same backend `_proxy_runner_get` helper. Writes/streams/SSE/blob-downloads stay on `/api/sandbox` (the InferencePool lane never truncates those). See `control-panel.md` for the full lane.
