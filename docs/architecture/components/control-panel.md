...
> **As-built increment — self-registration approval + runtime edit (2026-06-24).** Two surfaces shipped on the slice. (1) **Public self-registration:** `POST /api/auth/register` (unauthenticated, sibling of `/setup` and `/login`) validates the username is free + has no open request, bcrypts the chosen password, and stores a `pending_registration` row via the data-plane. The admin queue lives under `/api/admin`: `GET /pending-registrations`, `POST /pending-registrations/{id}/approve` (creates the account from the stored hash, seeds `account_resource_spec`, and provisions the tenant with the requested runner type + resources), `POST …/{id}/reject`. The user-facing wizard is a 3-step modal off the login page; the admin sees a `Registered Users | Pending Approval` tab switch (one-click approve/reject). (2) **Runtime edit:** `PUT /api/admin/users/{username}` now also accepts `agent_runner_type` + `cpu_cores`/`memory_mb`/`volume_gb`; it writes data-spine and then calls the provisioner's `update_tenant_runtime` to patch the `AgentTenant` CR so the operator applies the change live. **Auth note (as-built):** no new "admin key" guard was needed — the existing `require_admin` already accepts a bearer token that is *either* an admin JWT *or* an admin's account `api_key` (`authenticate_raw_token` steps 1–2), satisfying the "JWT **or** admin api-key" requirement.

> **As-built increment — `/sandbox` namespace + agent-runner OpenAPI docs (2026-06-28).** The agent-runner's runtime API moved from a flat `/api/*` to **`/api/sandbox/*`**, so the edge `HTTPRoute` collapses from ~17 enumerated runtime prefixes to a **single** `{type: PathPrefix, value: /api/sandbox}` rule → InferencePool; the control-plane (`/api/auth`, `/api/admin`, `/api/resource/models`, `/metrics`) and the SPAs stay on the `/` catch-all → this app. The **user SPA is now served at both `/` and `/sandbox`** (two `StaticFiles` mounts of the same build; vite `base` stays `/`). The agent-runner's API reference is **Scalar** (offline, vendored bundle), enabled on the pod at **`/sandbox/apidocs`** (HTML + `/sandbox/apidocs/scalar.js` + the `/sandbox/apidocs/openapi.json` schema), but **served by this app**, not the pool: the GIE/EPP response path buffers bodies to ~8KB and would truncate the ~91KB schema (and the 3.7MB bundle). control-panel adds a `/sandbox/apidocs*` route (`app.py`) that **proxies from any Ready runner** (`provisioner.any_ready_runner_endpoint()`; account-independent, the same pod-to-pod hop the EPP already makes for liveness; cache headers forwarded) — full body, on the `/` catch-all, unauthenticated (only the API *shape* is exposed, never user data). The user SPA's "API Doc" link opens `/sandbox/apidocs` in a new tab.

> **As-built increment — generic large-body read lane `/api/cp-proxy` (2026-06-30, shipped + verified live).** The ~8KB truncation that forced `/sandbox/apidocs` off the pool (above) is **not** apidocs-specific. agentgateway v1.3.0's GIE `InferencePool` routes *every* response body through the control-panel EPP `ext_proc`, and `InferenceRouting::build()` **hardcodes** `response_body_mode=FullDuplexStreamed` with `allow_mode_override=false` — it is **not** a tunable buffer and has **no** upstream fix (byte-identical on `main` and `v1.3.1`; the EPP's own `mode_override=NONE` is ignored). So *any* per-account GET over `/api/sandbox/*` larger than ~8KB truncates mid-body and breaks `JSON.parse` on the client (session transcripts 35–300KB, file previews up to 1MB, big list/JSON bodies). **The fix** is a generic control-panel reverse-proxy lane that rides the `/` catch-all (which carries **no** `ext_proc`, so it returns full bodies): `GET /api/cp-proxy/{sandbox_path:path}` (`app.py`, `include_in_schema=False`) rejects any path containing `..` (**400**) then calls the shared helper `_proxy_runner_get(request, sandbox_path)`, which **re-does the EPP's per-account gating**: bearer token → `authenticate_raw_token` (None / no `account_id` → **401**; account `status != "active"` → **403**, self-account only), `provisioner.wake_and_wait(account_id)` (exception → **503**, empty → **503**), then a **GET-only** hop to `http://{endpoint}/api/sandbox/{sandbox_path}` (`?{query}` forwarded) carrying a **per-account** `X-Priva-Runner-Token` minted via `runner_token.mint(account_id, username)` — so a caller can reach **only their own pod** — over `httpx.AsyncClient(trust_env=False, timeout=30.0)` (the in-cluster pod-to-pod hop must not honor any host/system proxy; `r.content` is buffered fully, fine for the ≤1MB preview cap; upstream exception → **502**). Because `/api/cp-proxy/*` is **not** under `/api/sandbox`, Gateway-API most-specific-prefix routing sends it to the `/` catch-all → control-panel `:8080` automatically — **no `deploy/gateway` / `HTTPRoute` edit needed** (the same lane that already serves the SPAs + `/sandbox/apidocs`). The pre-existing one-off `GET /api/session-history/{session_id}/messages` proxy is now a thin **back-compat alias** that just calls `_proxy_runner_get(request, "agent/sessions/{id}/messages")`; the `/sandbox/apidocs` proxy is **unchanged** (separate, account-independent, unauthenticated, proxied from any Ready runner via `any_ready_runner_endpoint`). **Frontend** (`web/shared/api/client.js`): a new `sandboxRead(path)` tries `fetchWithWake('/api/cp-proxy'+path)` and returns `handleAPIResponse` unless `res.status === 404`; on **404** (route not yet deployed) **or** a network error it falls back to the direct `fetchWithWake(SANDBOX_BASE+path)` lane — same signature as `sandboxGet(path)`, so rerouting a read is a drop-in `sandboxGet → sandboxRead` swap that degrades gracefully. **Rollout is incremental.** *Step 1 (DONE — shipped via a control-panel **image rebuild**, the backend is not hotloadable):* rerouted `web/user/src/api/sessions.js` only — `fetchSessionsGrouped` / `fetchCwdSessions` / `fetchArchivedSessions` and `fetchSessionMessages` (now `sandboxRead('/agent/sessions/{id}/messages')`, replacing its old session-history-primary + sandbox-fallback shape). *Step 2 (PENDING user OK; frontend-only + **hotloadable**, since the route already shipped):* reroute the remaining >8KB-risk reads via the same swap across `web/user/src/api/` (`userFiles`/`files` listings + preview, `hooks` logs/script, `mcp`, `skills`, `skillHub`, `subagents`, `settings` models, `userData` overview/stats/audit/analytics). **Never rerouted** (stay on the direct `/api/sandbox` lane in both steps): all writes (POST/PUT/DELETE/PATCH); all streaming/SSE/WS (agent run/stream, terminal pty); blob downloads; the control-plane `getControlPlaneAudit` (`/api/auth/audit`); `getAgentHealth` (the readiness/bootstrap poll, must hit the pod directly); and always-tiny configs (creds, quickactions, vision, small hook/skill/settings bodies) — streams ride the unchanged `InferencePool` lane because the EPP is consulted only at WS *setup* (steering), not in the byte path. **Verified live (minikube, 2026-06-30, account `admin`):** a 142KB transcript returned **8063 bytes** with `JSON.parse` **FAILED** ("Unterminated string") over the `/api/sandbox` pool lane, but the full **142937 bytes** of valid JSON (Content-Length present) over `/api/cp-proxy`; a tokenless `/api/cp-proxy/…` → **401** "Authentication required" with `server: uvicorn` (proving it rode the `/` catch-all to control-panel, **not** the InferencePool); a GET on the POST-only stream route still returned **405** (the InferencePool lane steers to the runner intact).
...
| **HTTP — internal** | `:8081` | scheduler, operator, **channel-connector**, self | `PushToChannel` seam (agent-gateway §8.4), health, and the **connector→brain routing RPC** — the IM-side `resolve·mint·wake·steer` seam (agent-gateway §4.4); same factored brain logic as `ext_proc`, different transport |

The **channel-connector** (the WeCom socket owner, agent-gateway §5) **stays a separate Deployment** — it cannot fold into this app because it dials/holds an **outbound** socket under a single-owner lease (agentgateway is inbound-only; §1.1, agent-gateway §5). Its **inbound IM bypasses agentgateway** (it owns the WeCom socket directly); it drives the brain through the **internal routing RPC** (`:8081`, §0.1; agent-gateway §4.4) and **dials pods directly** (mTLS + signed `account_id`), sharing the same Redis/data-plane (agent-gateway §5 unchanged).
...
└── src/priva_control_panel/
    ...
    ├── app.py                           # FastAPI factory; lifespan (extproc + optional data-plane); SPA mounts (env→_web→repo); large-body GET proxies — /api/cp-proxy + /api/session-history (alias) via _proxy_runner_get, /sandbox/apidocs via any_ready_runner
