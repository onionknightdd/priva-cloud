...
> **As-built increment — `/sandbox` namespace + agent-runner OpenAPI docs (2026-06-28).** The agent-runner's runtime API moved from a flat `/api/*` to **`/api/sandbox/*`**, so the edge `HTTPRoute` collapses from ~17 enumerated runtime prefixes to a **single** `{type: PathPrefix, value: /api/sandbox}` rule → InferencePool; the control-plane (`/api/auth`, `/api/admin`, `/api/resource/models`, `/metrics`) and the SPAs stay on the `/` catch-all → this app. The **user SPA is now served at both `/` and `/sandbox`** (two `StaticFiles` mounts of the same build; vite `base` stays `/`). The agent-runner's API reference is **Scalar** (offline, vendored bundle), enabled on the pod at **`/sandbox/docs`** (HTML + `/sandbox/docs/scalar.js` + the `/sandbox/docs/openapi.json` schema), but **served by this app**, not the pool: the GIE/EPP response path buffers bodies to ~8KB and would truncate the ~91KB schema (and the 3.7MB bundle). control-panel adds a `/sandbox/docs*` route (`app.py`) that **proxies from any Ready runner** (`provisioner.any_ready_runner_endpoint()`; account-independent, the same pod-to-pod hop the EPP already makes for liveness; cache headers forwarded) — full body, on the `/` catch-all, unauthenticated (only the API *shape* is exposed, never user data). The user SPA's "API Doc" link opens `/sandbox/docs` in a new tab.
