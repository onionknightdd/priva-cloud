---
Status: Draft · Date: 2026-06-17 (M7: 2026-07-11) · Branch: multi-tenant-platform
Parent: ../multi-tenant-platform.md · Component: Data Spine (SQL-service [SQLite or Postgres] + Redis)
Modifications from blueprint: M1 = SQLite+Redis (not Postgres); M2 = no CLI session-id remap; M7 = Postgres backend restored (reverses M1's SQLite half; the service boundary stands)
---
...
- **M7 — Postgres backend lands; M1's "SQLite, not Postgres" half is reversed (2026-07-11).** The storage seam absorbed it as designed: `Repository` (ABC, `services/data-spine/src/priva_data_spine/repo.py`) now has two production implementations — `SqliteRepo` (unchanged; legacy opt-in, see the defaults addendum below) and **`PgRepo`** (psycopg3 sync + thread-safe connection pool; the pool replaces the RLock — Postgres handles concurrency natively) — selected by `dataspine.backend` with the libpq DSN in **`dataspine.postgres_dsn`** (`PRIVA_DATASPINE__POSTGRES_DSN`, Secret-sourced env on the **data-spine pod ONLY**: the DSN carries DB credentials, tenant pods share the namespace and expose env via the web terminal, and a NetworkPolicy admits only data-spine to :5432). The four formerly-unconditional `compose()` sites (monolith lifespan, agent-runner app, scheduler/channels daemons) are now **gated on `transport == "in_process"`**, so a gRPC-client process can never construct a repo. **Schema parity is deliberate** (`schema_pg.py`): TEXT ISO-8601 timestamps and 0/1 integer booleans survive on PG, so the service layer, keyset pagination, and retention cutoffs stay backend-blind. **What M1 keeps:** the single data-plane service boundary (typed RPC; callers never see SQL) — never SQLite-specific — and `replicas:1` for the alpha, though PG no longer structurally requires it. **What M1 loses:** the fcntl/NFS-corruption, WAL-single-writer, and RWO-PVC/`Recreate` split-brain rationale no longer applies to the DB; the SQLite file + PVC stay mounted only as the **`migrate-to-pg` source and instant rollback target** (flip the env back; PG-side writes after cutover do not flow back). The idempotent **`data-spine migrate-to-pg`** CLI copies rows byte-for-byte (bcrypt hashes, encrypted api_keys, and HMAC lookups stay valid iff the same `api_key_hmac_secret`/`jwt_secret` is configured on both sides; run it with the writer idle). **Redis is untouched** — M7 moves the SQL half only; the routing/awake/lock hot paths stay Redis (§4). Deploy: in-cluster `postgres:16` on a local/block RWO PVC, **never the NFS export** (`deploy/k8s/postgres.yaml`; helm `postgres.enabled` + `dataSpine.backend=postgres`). DR migrates from SQLite-PVC snapshots to the PG PVC (CSI snapshot or a `pg_dump` CronJob — **open pre-UAT TODO**). §1.4's PRAGMA contract and §1.5's Litestream/rqlite ladder below are SQLite-backend-only text. **Defaults addendum (2026-07-11, same day): Postgres is the DEFAULT everywhere, not just this cluster's state** — `DataspineSettings.backend` defaults to `"postgres"` (a missing DSN fails fast with an actionable error incl. a local `docker run postgres:16-alpine` one-liner), the helm chart ships `postgres.enabled=true` + `dataSpine.backend=postgres` (base/UAT/prod values), and the config example is PG-first. SQLite is an **explicit legacy opt-in** (`PRIVA_DATASPINE__BACKEND=sqlite`), retained solely as the `migrate-to-pg` source and rollback target. The test suite pins the sqlite backend in `tests/conftest.py` so a bare `pytest` stays infrastructure-free; PG coverage runs via the backend-parametrized dataplane tests under `TEST_POSTGRES_DSN`.

> **As-built increment — runner type, resource specs & self-registration (2026-06-24; what actually ships on the minikube slice).** The as-built SQLite schema (`services/data-spine/src/priva_data_spine/schema.py`) is the slim 6-table footprint plus **two new tables** and one new `account` column — simpler than the full §2 blueprint, additively. (1) `account` gains **`agent_runner_type TEXT NOT NULL DEFAULT 'auto_scale' CHECK (… IN ('auto_scale','persistent'))`**, added idempotently (a guarded `ALTER TABLE` runs in `create_all` when the column is missing — there is no migration framework). (2) **`account_resource_spec`** (PK `account_id` → `account` `ON DELETE CASCADE`; `cpu_cores REAL`, `memory_mb INTEGER`, `volume_gb INTEGER`) — per-account pod sizing the operator reads. (3) **`pending_registration`** (PK `request_id`; `username`, `password_hash` bcrypt, `display_name`, `runner_type`, `cpu_cores`/`memory_mb`/`volume_gb`, `note`, `status` ∈ pending|approved|rejected; partial unique index `WHERE status='pending'`) — self-service requests awaiting admin approval; **deliberately not** FK'd to `account` (the account does not exist until approval). New gRPC services `ResourceSpecService` + `RegistrationService` (`protos/.../resource_spec.proto`, `registration.proto`); `AccountService.Create` now also accepts a precomputed `password_hash` so the approval path mints the account from the stored bcrypt directly. `password_hash` is never returned by `Registration.List` — only by the internal `Get` the approve path reads.
