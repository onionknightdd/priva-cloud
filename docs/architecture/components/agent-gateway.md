...
IM(WeCom) WeCom ─(leased socket)→ channel-connector ─(internal call)→ BRAIN (resolve·mint·wake·steer)
...
- **Open item (minor):** the connector→brain call mechanism — an internal RPC to the brain's internal seam (default, keeps one brain authority) vs. re-injecting the normalized turn through agentgateway as a synthetic `/v1` request — is confirmed in the channel-connector's detailed pass; the brain *logic* is identical either way.
