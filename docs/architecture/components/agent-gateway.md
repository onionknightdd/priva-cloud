...
> **As-built increment — the EPP/`InferencePool` response-body cap + the generic `/api/cp-proxy` read lane (Step 1 shipped + live-verified on minikube 2026-06-30).** The "agentgateway streams the bytes through (browser)" claim above holds for **streaming** (WS/SSE): the EPP is consulted only at WS **setup** (the control-panel.md §2.3 steer), never in the frame byte path, so frames tunnel browser⇄agentgateway⇄pod intact (verified — a `GET` on the POST-only stream route still 405s via the pool, steering live). It does **not** hold for **buffered (non-streaming) `GET` bodies** on the `/api/sandbox` `InferencePool` lane: that lane routes **every** response body through the EPP `ext_proc`, and agentgateway v1.3.0 **hardcodes** the EPP's `response_body_mode=FullDuplexStreamed` (`allow_mode_override=false`, in `InferenceRouting::build()`) — **not** a tunable buffer, **no** upstream fix (byte-identical on `main` and v1.3.1), and the EPP cannot recover the bytes (its own `mode_override=NONE` is ignored). So any response over **~8KB** is truncated and breaks client `JSON.parse` ("Unterminated string") — affecting session transcripts (35–300KB), file previews (≤1MB), and large list/JSON bodies. *(Full root-cause lives in control-panel.md's as-built increment banners — the 2026-06-28 `/sandbox` namespace note and the 2026-06-30 `/api/cp-proxy` lane note — plus its §2.3 EPP-steer mechanism; not duplicated here.)*
>
> **Mitigation (production — `services/control-panel/src/priva_control_panel/app.py`):** a **generic** control-panel read lane, `GET /api/cp-proxy/{sandbox_path:path}`, that rides the **`/` catch-all** (which carries **no** `ext_proc`, so it returns the **full** body). The shared `_proxy_runner_get` helper re-does the EPP's per-account steering — bearer token → `authenticate_raw_token` (else **401**), self-account `status=="active"` (else **403**), `wake_and_wait(account_id)` (else **503**), mint a per-account `X-Priva-Runner-Token` — then proxies **GET-only** to the caller's own pod at `/api/sandbox/{path}`, rejecting any `..` (**400**). Security parity with the EPP: a caller reaches **only their own** pod. This subsumes the one-off session-transcript proxy — the old `GET /api/session-history/{id}/messages` is now a thin **back-compat alias** over the helper; the separate `/sandbox/apidocs` proxy is **unchanged** (its account-independent sibling, proxied from any Ready runner, unauthenticated, API shape only). **No `HTTPRoute`/`deploy/gateway` change** is needed — `/api/cp-proxy/*` is not under `/api/sandbox`, so Gateway-API most-specific-prefix routing sends it to the `/` catch-all → control-panel automatically (the same lane that serves the SPA). The SPA helper `sandboxRead(path)` (`web/shared/api/client.js`) tries `/api/cp-proxy`+path and falls back to the direct `/api/sandbox` lane on a 404 (route not yet deployed) or network error — a drop-in `sandboxGet → sandboxRead` swap that degrades gracefully.
>
> **Scope (incremental):** only **>8KB-risk GET reads** reroute. **Step 1 (shipped, via a control-panel image rebuild — the backend is not hotloadable):** `web/user/src/api/sessions.js` only (`fetchSessionsGrouped` / `fetchCwdSessions` / `fetchArchivedSessions` / `fetchSessionMessages`). **Step 2 (pending; frontend-only + hotloadable, since the backend route already shipped):** the remaining large reads (files/hooks/mcp/skills/subagents/settings/userData). **Never rerouted — they stay on the `/api/sandbox` `InferencePool` lane:** all writes (POST/PUT/DELETE/PATCH), all streaming (run WS + run/stream SSE, terminal pty WS), blob downloads, the control-plane `/api/auth/audit`, the `/health` readiness poll, and always-tiny configs.
...
> **As-built caveat (2026-06-30):** this wake-free model is the **blueprint target, not yet realized** — there is no `ListSessionsByAccount` RPC and no running `state-reader` (it survives only as a System-Map node label and a `future` export-PV mount). The shipped session list (`/api/cp-proxy/agent/sessions`) and transcript reads (`/api/cp-proxy/agent/sessions/{id}/messages`, §4.2) both ride `sandboxRead` (cp-proxy-first) and are served by the **live agent-runner** off its own volume, so today they **do** wake the pod via the helper's `wake_and_wait`. See ../byte-path.md and control-panel.md.
