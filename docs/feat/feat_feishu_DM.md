...
（`/new` `/clear` `/compact` `/skill` `/info` `/help`）+ 四按钮
`[🆕 开始新对话] [🧹 /clear] [📦 /compact] [🧩 /skill]`，其余内容与欢迎卡一致。

两张卡共用一套文案（2026-07-28 统一）：不列别名（`/新` `/reset` `/帮助` `/skills`
`/技能` `/信息` 代码里照常生效，只是不占卡片篇幅），指令列表下方是同一句
