...
// Scope chooser for "New command" — mirrors the subagents scope picker. USER
// ($CLAUDE_CONFIG_DIR/commands) at the top, then one row per project workdir
// writing {cwd}/.claude/commands, plus a Browse… escape hatch.
export default function CommandScopePicker() {
  const { t } = useTranslation()
  const open = useCommandsStore((s) => s.scopePickerOpen)
  const closeScopePicker = useCommandsStore((s) => s.closeScopePicker)
  const chooseScope = useCommandsStore((s) => s.chooseScope)
          ...
          <div className="flex flex-col gap-0.5 px-3 py-2 overflow-y-auto" style={{ minHeight: 0 }}>
            <button style={rowStyle} onMouseEnter={onIn} onMouseLeave={onOut}
              ...
              <div className="flex flex-col min-w-0 flex-1">
                ...
                <span className="truncate" style={subtitle}>~/.claude/commands</span>
              </div>
            ...
            <button style={{ ...rowStyle, marginTop: 4, borderTop: '1px solid var(--border-subtle)', borderRadius: 0 }}
              ...
              <span className="flex-1 truncate" style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{t('commands.browse')}</span>
              <ChevronRight size={14} strokeWidth={1.5} style={{ flexShrink: 0, color: 'var(--text-dim)' }} />
            </button>
          </div>
        </div>
