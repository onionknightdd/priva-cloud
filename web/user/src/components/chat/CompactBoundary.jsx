...
export default function CompactBoundary({ message }) {
  ...
  // Compacting pulse → completed rule: the skeleton lifts + fades while the
  // final boundary rises into place (both states share one relative wrapper).
  const compacting = status === 'compacting'
  const handoff = useSkeletonHandoff(compacting)
